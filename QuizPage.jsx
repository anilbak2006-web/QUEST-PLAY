...
function QuizPage({ quiz, onBack, setQuizTotalCount }) {
  ...
  function finishQuiz() {
    ...
    if (Object.keys(answers).length < quiz.questions.length) {
      setMessage("Lütfen Quiz'i bitirmeden önce Tüm soruları cevapla !");
      return;
    }


    // Switch from the question form to the appropriate result screen.
  ...
  function HandleBack() {
    ...
    if (QuizFinished) {
      setQuizTotalCount(prevent => prevent + 1);
            ...
            {quiz.questions.map((question, index) => (
              <div key={index}>
                ...
                {question.options.map((option) => {
                  ...
                  const details = question.optionDetails?.[letter];

                  return (
                    <button
                      className={`OptionStyles ${answers[index] === option
                        ? "selectedOption"
                        : ""
                        }`}
                      key={option}
